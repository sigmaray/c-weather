...
static void on_settings_save(GtkButton *button, gpointer user_data) {
    ...
    gchar *provider =
        gtk_combo_box_text_get_active_text(GTK_COMBO_BOX_TEXT(w->provider));
    ...
    /* Prefer city+country when both location modes are filled. */
...
void ui_show_settings(GtkWindow *parent) {
    ...
    w->provider = gtk_combo_box_text_new();
    w->api_key = gtk_entry_new();
    gtk_entry_set_visibility(GTK_ENTRY(w->api_key), FALSE);

    gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(w->provider),
                                   "open-meteo");
    gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(w->provider),
                                   "openweathermap");
